...
def getAccouts(user):
    ...
    conn = MySQLdb.connect(host=settings.HOST_IP, \
                           ...
                           db="keystone")
    cursor = conn.cursor()
    cursor.execute("select tenant.name from tenant left join user_tenant_membership on user_tenant_membership.tenant_id = tenant.id left join user on user.id  = user_tenant_membership.user_id where user.name = '"+user+"'")
    res = cursor.fetchall()
    cursor.close()
    conn.close()
    return res
...
#     print aus.users.findall()
    conn = Connection(authurl = 'http://localhost:5000/v2.0/',user = 'admin',key = 'ADMIN',tenant_name = 'admin')
...
if __name__ == '__main__':
    print get_auth(AUTH_URL, 'admin', 'ADMIN')
    test()
